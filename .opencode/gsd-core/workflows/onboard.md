...
@/home/diplo/projects/game-of-life/.opencode/gsd-core/references/gsd-run-resolver.md
...
- `--text` forces text-mode choices for runtimes without `question`.
...
Set:
- `TEXT_MODE=true` if `--text` is present or `text_mode` is true. When `TEXT_MODE` is active, replace every `question` call below with a plain-text numbered list and ask the user to type their choice number — required for non-the agent runtimes (OpenAI Codex, Gemini CLI, etc.) where `question` is not available.
