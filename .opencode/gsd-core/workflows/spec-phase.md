...
ones). Reference: @/home/diplo/projects/game-of-life/.opencode/gsd-core/references/edge-probe.md.
...
# — NOT the consuming project's git root — falling back to git toplevel / /home/diplo/projects/game-of-life/.opencode.
...
EDGE_PROBE_JS=$(for _c in \
  ...
  "/home/diplo/projects/game-of-life/.opencode/gsd-core/bin/lib/edge-probe.cjs" \
  "/home/diplo/projects/game-of-life/.opencode/bin/lib/edge-probe.cjs"; do
  [ -f "$_c" ] && { echo "$_c"; break; }
done)
...
If `$APPLICABLE` is `0`, do NOT proceed silently: ask the author to confirm via question
...
3. Resolve each with the user (question; text mode → numbered list):
...
requirement list). Reference: @/home/diplo/projects/game-of-life/.opencode/gsd-core/references/prohibition-probe.md — the
...
4. **Resolve each surfaced (non-canon) prohibition** (question; text mode → numbered list):
...
**Text mode (PROB-09):** per Step 5's text-mode rule, replace the question menus above
with plain-text numbered lists — there is NO hard question dependency, so the probe
...
Use the SPEC.md template from @/home/diplo/projects/game-of-life/.opencode/gsd-core/templates/spec.md.
