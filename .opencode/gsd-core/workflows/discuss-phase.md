...
<required_reading>
@/home/diplo/projects/game-of-life/.opencode/gsd-core/references/domain-probes.md
@/home/diplo/projects/game-of-life/.opencode/gsd-core/references/gate-prompts.md
@/home/diplo/projects/game-of-life/.opencode/gsd-core/references/universal-anti-patterns.md
</required_reading>
...
if [ -f "/home/diplo/projects/game-of-life/.opencode/gsd-core/USER-PROFILE.md" ]; then
  ADVISOR_MODE=true
else
  ADVISOR_MODE=false
fi
...
**If matches found:** Present each match (title, area, why it matched). question (multiSelect) asking which to fold. Folded → `<folded_todos>` for CONTEXT.md `<decisions>`. Reviewed but not folded → `<reviewed_todos>` for CONTEXT.md `<deferred>`.
...
Read `@/home/diplo/projects/game-of-life/.opencode/gsd-core/references/scout-codebase.md` — it contains the phase-type→map selection table, single-read rule, no-maps fallback, and `<codebase_context>` output schema. Then execute:
...
```
Apply each entry in `activeHooks` per @/home/diplo/projects/game-of-life/.opencode/gsd-core/references/loop-hook-dispatch.md. Empty list → continue to `analyze_phase`.
</step>
...
- **--auto:** follow `workflows/discuss-phase/modes/auto.md` — the agent picks recommended option for every question; no question. Single-pass cap enforced.
...
```
Apply each entry in `activeHooks` per @/home/diplo/projects/game-of-life/.opencode/gsd-core/references/loop-hook-dispatch.md. Empty list → continue to `confirm_creation`.
</step>
